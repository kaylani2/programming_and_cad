 ...
 *  
 *  const uint8_t asciimap[128] is defined on the Keyboard.cpp file
 * 
 *  On Windows 10 is located under C:\Program Files (x86)\Arduino\libraries\Keyboard\src
 * 
 *  There are problems when trying to use the Duckie on computer without a english layout configuration
 *  Maybe the asciimap array can have it's values changed to fix this, haven't tested it yet.
 * 
 * 
 */

/*
 * Based on the code written by Seytonic
 * The code will use a five way dip switch to choose between 2^5 (32) different scripts
 * We'll use the 11111 script as programming mode, so the 11111.duc file in the sd card will be empty
*/
...
void setup() {
  Serial.begin(9600);
  String dipString = ""; // Name of the file that will be opened
  ...
  scriptFile = SD.open(dipString);
  if (scriptFile) {
    Keyboard.begin();

    String fullCommand = "";
    while (scriptFile.available()) {
      char charCommand = scriptFile.read();
      if (charCommand == '\n') {
        Line(fullCommand);
        fullCommand = "";
      }
      else if ((int) charCommand != 13)
      {
        fullCommand += charCommand;
      }
    }
    Line(fullCommand);
...
/*
  //
  //  //using echo a > COM9 on the command prompt will trigger the keyboard.println command
  if (Serial.read() == 'a')
  {

    delay(10000);
    Serial.write("ok");
    Keyboard.println("hahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahahaha");
  }
*/
